defmodule CleanCode.Orders do
  ...
  # just to understand how typespec work, will remove this method soon
  # CleanCode.Orders.really_create_order(CleanCode.Repo)
  # def really_create_order(repo) do
  #   customer = CleanCode.Customers.get_customer(CleanCode.Repo, 1)
  #   event = CleanCode.Events.get_event(CleanCode.Repo, 1)
  ...
  @spec paid(Ecto.Queryable.t()) :: Ecto.Query.t()
  defp paid(query) do
    from(order in query, where: order.status == "PAID")
  end

  @spec pending_payment(Ecto.Queryable.t()) :: Ecto.Query.t()
  defp pending_payment(query) do
    from(order in query, where: order.status == "PENDING_PAYMENT")
  end

  @spec for_event(Ecto.Queryable.t(), Event.t()) :: Ecto.Query.t()
